...
########################################################
	...
	### COUNT EVENTS OPTIONS
		...
		# nodes of interest
			...
			# each node of interest must be defined as a vector within the list
			# if you don't want to tag the nodes of interest, define an empty list: nodes_of_interest <- list(), names(nodes_of_interest) <- c()
	...
	if (sample_trees==TRUE & run_BGB == TRUE & checkpoint_from_previous_run != TRUE) {
		...
		n_polytomous <- 0
			if(n_trees == 1) { # if this is a single target tree...
				...
				species_to_drop_all <- c(species_to_drop_all, species_to_drop)
		...
		species_to_drop_all <- sort(unique(species_to_drop_all))
		message("Taxa dropped due to not being listed in geography file:")
		species_to_drop_all
		write(c("Taxa dropped due to not being listed in geography file:",species_to_drop_all), "dropped_taxa.txt")
...
##########################################################################
	...
	# only proceeds with analysis if everything is OK
		...
		for (r in 1:n_trees) {
			...
			#check if all species in the distribution file are present in the tree
				if(FALSE %in% (species %in% tr$tip.label)){
					stop("The distribution file contains species not present in the tree.")
				}
			...
			#gets the node number for clades of interest for this particular tree
				...
				#checks if there is something wrong with this..
				if(list(NULL) %in% node_interest_n){
					stop("Object nodes_of_interest incorrectly defined; please revise.")
				}		
