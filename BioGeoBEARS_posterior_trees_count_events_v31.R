	...
	### RUN OPTIONS
		...
		#biogeographic stochastic mapping
			...
			#number of BSM to run per tree
				n_stochastic_maps = 10
		#number of cores to be used in BioGeoBEARS likelihood estimations	
			n_cores <- 2
		#method for searching for optimal parameters during ML calculations
	...
	### TREES
		...
		#number of trees to be sampled from the posterior distribution
			...
			# or any other single, target tree
				n_trees = 10
		#TRUE samples n_trees from trees file, FALSE uses trees previously sampled and saved as /trees/tree1.nex, /trees/tree2.nex...
			sample_trees = TRUE
...
##################################################
	...
	if (sample_trees==TRUE & run_BGB == TRUE) {
		tic()
		...
		n_polytomous <- 0
			if(n_trees == 1) { # if this is a single target tree...
				...
				for(i in 1:length(pruned_trees)) {
					...
					# this might fix this but is not necessary in most cases, so the lines are commented. Hope you don't need them!
	...
	if (plot_LTT) {
		...
		for (t in 0:(length(TimePoint)-1)) {
			for (n in 1:length(names_single_areas_)){
				...
				averageLTTbyarea[t+1,names_single_areas_[[n]]] <- mean_LTT
				if (length(row_time_period)>1){
					error_LTT <- sd(mean_by_time_period)/sqrt(length(row_time_period))
					low95 <- mean_LTT -(2*error_LTT)
					if(low95 < 0) {averageLTTbyarea_95_lower[t+1,names_single_areas_[[n]]] <- 0} else {averageLTTbyarea_95_lower[t+1,names_single_areas_[[n]]] <- low95}
					averageLTTbyarea_95_upper[t+1,names_single_areas_[[n]]] <- mean_LTT +(2*error_LTT)
				} else {
					averageLTTbyarea_95_upper[t+1,names_single_areas_[[n]]] <- mean_LTT
					averageLTTbyarea_95_lower[t+1,names_single_areas_[[n]]] <- mean_LTT
				}					
			}
		}
	}
